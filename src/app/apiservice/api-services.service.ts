...
export class ApiServicesService {
  ...
  public getAllProductspage<T>(page?: number): Observable<T> {
    ...
    const serviceURL = `${this.urlHelper.getAPIURL()}${this.envUrl.getAdminReviews}/${productId}${query}`;
    return this.http.get<T>(serviceURL, { headers: this.getHeaders() }).pipe(
      catchError((error) => {
        if (error.status === 403) {
          sessionStorage.clear();
  ...
  public approveReview<T>(id: number, isApproved: boolean): Observable<T> {
    ...
    return this.http.put<T>(serviceURL, { is_approved: isApproved }, { headers: this.getHeaders() }).pipe(
      catchError((error) => {
        if (error.status === 403) {
          sessionStorage.clear();
          localStorage.clear();
  ...
  public addQuickHit<T>(payload: { product_id: number; position?: number }): Observable<T> {
  ...
  public removeQuickHit<T>(productId: number): Observable<T> {
    ...
    return this.http.post<T>(serviceURL, { product_id: productId }, { headers: this.getHeaders() }).pipe(
      catchError((error) => {
        if (error.status === 403) {
          sessionStorage.clear();
          localStorage.clear();
  ...
  public toggleQuickHit<T>(productId: number, isActive: boolean): Observable<T> {
    const serviceURL = `${this.urlHelper.getAPIURL()}${this.envUrl.toggleQuickHit}`;
    return this.http.post<T>(serviceURL, { product_id: productId, is_active: isActive }, { headers: this.getHeaders() }).pipe(
      catchError((error) => {
        if (error.status === 403) {
          sessionStorage.clear();
          localStorage.clear();
  ...
  public reorderQuickHits<T>(order: { product_id: number; position: number }[]): Observable<T> {
    const serviceURL = `${this.urlHelper.getAPIURL()}${this.envUrl.reorderQuickHits}`;
    return this.http.post<T>(serviceURL, { order }, { headers: this.getHeaders() }).pipe(
      catchError((error) => {
        if (error.status === 403) {
          sessionStorage.clear();
          localStorage.clear();
  ...
  public updateReelStatus<T>(id: number, isPublished: boolean): Observable<T> {
    ...
    return this.http
      .post<T>(serviceURL, { id, is_published: isPublished }, { headers: this.getHeaders() })
      .pipe(
        catchError((error) => {
          if (error.status === 403) {
            sessionStorage.clear();
  ...
  public updateOrderStatus<T>(orderId: string, status: string): Observable<T> {
    ...
    return this.http
      .put<T>(serviceURL, { order_id: orderId, status }, { headers: this.getHeaders() })
      .pipe(
        catchError((error) => {
          if (error.status === 403) {
            sessionStorage.clear();
