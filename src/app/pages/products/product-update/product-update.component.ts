...
interface SizeRow {
  id?: number | null; // existing inventory id, null for newly added rows
  size: string;
  sku: string;
  price: number | null;
  stock: number | null;
...
interface ColorVariant {
  id?: number | null; // existing product_color id, null for newly added variants
  color_id: number;
  name: string;
  code: string;
  images: string[];
...
export class ProductUpdateComponent implements OnInit {
  ...
  private tryFetchProduct(): void {
    ...
    if (!this.loadingColors && !this.colorsReady) {
      this.colorsReady = true;
    }
    if (this.categoriesReady && this.colorsReady) {
  ...
  getProductById(data: any): void {
    this.apiServices.getAllProductsbyid(data).subscribe({
      ...
      error: (err: any) => {
        console.log(err);
        this.loadError = 'Failed to load product details. Please try again.';
        this.loadingProduct = false;
      }
    });
  }
  ...
  private hydrateForm(product: any): void {
    ...
    this.actual_price = product.actual_price !== null ? Number(product.actual_price) : null;
    this.selling_price = product.selling_price !== null ? Number(product.selling_price) : null;
    this.discount_percent = product.discount_percent !== null ? Number(product.discount_percent) : null;
    ...
    this.variants = (product.colors || []).map((c: any) => {
      ...
      return {
        ...
        selectedSizes: sizeRows.map(r => r.size),
        sizeRows
      } as ColorVariant;
    });
  ...
  removeColorVariant(index: number): void {
    ...
    if (variant?.id) {
      this.removedColorIds.push(variant.id);
    }
  ...
  uploadFiles(): void {
    ...
    this.apiServices.imageUpload(formData).subscribe({
      next: (res: any) => {
        ...
        const urls: string[] = uploaded.length ? uploaded.map(u => u.file_url) : [];
        if (urls.length) {
 ...
 private buildPayload(): any {
    return {
      ...
      colors: this.variants.map(v => ({
        product_color_id: v.id ?? null, // ✅ matches backend key
        color_id: v.color_id,
        images: v.images,
        sizes: v.sizeRows.map(r => ({
          inventory_id: r.id ?? null, // ✅ matches backend key
          size: r.size,
          sku: r.sku,
          price: r.price,
          stock: r.stock ?? 0
